...
Object.keys(allSelects).forEach(function(cat){const catName=d(cat);allSelects[cat].forEach(function(id){const prix=parseFloat(t("#"+id).val())||0;if(prix>0){b[catName]||(b[catName]=[]);const label=t("#"+id).data("label")||t("#"+id).find("option:selected").text().split("€")[0].trim();b[catName].push({desc:label,qty:1,unit:"u",price:prix})}})});
...
const R=[v,x].filter(Boolean).join(" ");y.text(R,110,D),D+=4,y.text("France",110,D),D+=4,y.text(h,110,D),D+=4,g&&y.text(g,110,D),k=95,y.setFontSize(11),y.setTextColor(0,0,0),y.setFont(void 0,"bold"),y.text("Devis Estimatif - Travaux de Rénovation",15,k),k+=8,y.setFillColor(74,144,226),y.rect(15,k-4,180,7,"F"),y.setTextColor(255,255,255),y.setFontSize(8),y.setFont(void 0,"bold"),y.text("N°",17,k),y.text("DÉSIGNATION",27,k),y.text("QTÉ",115,k),y.text("PRIX U.",135,k),y.text("TVA",155,k),y.text("TOTAL HT",175,k),k+=5,y.setTextColor(0,0,0);
let E=0,z=1;Object.keys(b).forEach(t=>{const e=b[t];let o=0;e.forEach(t=>o+=t.qty*t.price),k>250&&(y.addPage(),k=20),k+=6,y.setFillColor(240,240,240),y.rect(15,k-4,180,6,"F"),y.setFont(void 0,"bold"),y.setFontSize(9),y.text(z+"",17,k),y.text(t.toUpperCase(),27,k),y.text(u(o),180,k);let n=1;e.forEach(t=>{k>270&&(y.addPage(),k=20),k+=5;const e=t.qty*t.price;E+=e,y.setFont(void 0,"normal"),y.setFontSize(8),y.text(z+"."+n,17,k);const o=t.desc.length>50?t.desc.substring(0,50)+"...":t.desc;y.text(o,27,k),y.text(t.qty+" "+t.unit,115,k),y.text(u(t.price),135,k),y.text("20 %",155,k),y.text(u(e),175,k),n++}),z++}),k+=15,k>250&&(y.addPage(),k=30),y.setDrawColor(74,144,226),y.setLineWidth(.5),y.line(120,k,190,k);
